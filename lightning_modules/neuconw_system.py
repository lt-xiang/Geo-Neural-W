...
class NeuconWSystem(LightningModule):
    ...
    def __init__(self, hparams, config, caches):
        ...
        self.config = config
        self.intrinsics_all = {}
        self.intrinsics_inv_all = {}
        self.poses_w2c_dict_all = {}
        self.intrinsics_all_val = {}
        self.intrinsics_inv_all_val = {}
        self.poses_w2c_dict_all_val = {}
        ...
        with open(os.path.join(self.root_dir, f'cache_sgs/poses_w2c_dict_all.pkl'), 'rb') as f:
            ...
            f.close()     
        with open(os.path.join(self.root_dir, f'cache_sgs/intrinsics_all_val.pkl'), 'rb') as f:
            self.intrinsics_all_val = pickle.load(f)
            f.close()
        with open(os.path.join(self.root_dir, f'cache_sgs/intrinsics_inv_all_val.pkl'), 'rb') as f:
            self.intrinsics_inv_all_val = pickle.load(f)
            f.close()
        with open(os.path.join(self.root_dir, f'cache_sgs/poses_w2c_dict_all_val.pkl'), 'rb') as f:
            self.poses_w2c_dict_all_val = pickle.load(f)
            f.close()    
        ...
        self.embedding_a = torch.nn.Embedding(self.config.NEUCONW.N_VOCAB, self.config.NEUCONW.N_A)
        self.embeddings["a"] = self.embedding_a
        self.models_to_train += [self.embedding_a]

        # define NEUCONW model
        self.neuconw = NeuconW(sdfNet_config=self.config.NEUCONW.SDF_CONFIG,
        ...
        spc_options = {"voxel_size": self.scene_config["voxel_size"],
    ...
    def forward(self, rays, ts, label,
                intrinsics, intrinsics_inv, poses_w2c_dict):
        
        """Do batched inference on rays"""
        results = defaultdict(list)
        rendered_ray_chunks = self.renderer.render( rays, ts, label,
    ...
    def surface_selection(self, train_level, threshold, device=0, chunk=65536):
        ...
        for i in tqdm(range(0, B, chunk), disable=local_rank != 0):
            new_sdf = self.renderer.sdf(local_xyz_[i : i + chunk].reshape(-1, 1, 3).cuda())
            out_chunks += [new_sdf.detach().cpu()]
        sdf = torch.cat(out_chunks, 0).reshape(-1)

        # if multi gpu
        if self.hparams.num_gpus > 1:
            sdf_gathered = [torch.zeros(B, dtype=torch.float, device=device) for _ in range(world_size)]
            dist.all_gather(sdf_gathered, sdf.cuda())
            sdf = torch.cat(sdf_gathered, 0).reshape(-1)[: xyz_training.size()[0]]

        # filter with threshold
        xyz_sfm = xyz_sfm.cuda()
        sparse_pc_sfm = xyz_sfm[sdf <= threshold].cpu().numpy()
        print(f"sdf filtered points {sparse_pc_sfm.shape[0]}, max sdf: {torch.min(sdf)}, min sdf: {torch.max(sdf)}")

        return sparse_pc_sfm, train_voxel_size

    def octree_update(self, train_level, threshold, device=0, chunk=65536, visualize=False):
    ...
    def training_step(self, batch, batch_nb):
        ...
        with torch.no_grad():
            if self.update_freq > 0 and (self.global_step + 1) % self.update_freq == 0:
                self.octree_update(self.train_level, self.sdf_threshold, device=rays.device)

        if (self.global_step % self.config.TRAINER.SAVE_FREQ == 0 and self.trainer.global_rank == 0):
            
            os.makedirs(f"{self.config.TRAINER.SAVE_DIR}/{self.hparams.exp_name}", exist_ok=True)
    
            if self.global_step == 0:
                # save config
                config_save_path = (f"{self.config.TRAINER.SAVE_DIR}/{self.hparams.exp_name}/config")
                ...
                for config_file in config_files:
                    if os.path.isfile(os.path.join(config_dir, config_file)):
                        copyfile(os.path.join(config_dir, config_file),
    ...
    def validation_step(self, batch, batch_nb):
        ...
        rays, rgbs, ts, labels = (batch["rays"],
                                  batch["rgbs"],
                                  batch["ts"],
                                  batch["semantics"])
        ...
        for i in range(0, B, self.hparams.test_batch_size):                
            rendered_ray_chunks = self( rays[i : i + self.hparams.test_batch_size],
        ...
        if batch_nb == 0 and self.global_step > 0:
            if self.trainer.global_rank == 0:
                ...
                img = (results[f"color"].view(H, W, 3).permute(2, 0, 1).cpu())  # (3, H, W)
                img_gt = rgbs.view(H, W, 3).permute(2, 0, 1).cpu()  # (3, H, W)
                depth = visualize_depth(results[f"depth"].view(H, W))  # (3, H, W)
                
                normal = ((torch.sum(results["gradients"][:, :, :] * results["weights"][:, : results["gradients"].size()[1],None
                            ],dim=1)).view(H, W, 3).detach().cpu())  # (H, W, 3)
                ...
                self.logger.experiment.add_images("val/GT_pred_depth_normal", stack, self.global_step)

            # save mesh
            mesh_dir = os.path.join(self.logger.save_dir, self.logger.name, "meshes")
            mesh = extract_mesh(dim=128,
                                chunk=16384,
            ...
            if self.trainer.global_rank == 0:
                mesh.export(os.path.join(mesh_dir, "{:0>8d}.ply".format(self.global_step)))

            sfm_to_gt = np.array(self.scene_config["sfm2gt"])
            gt_to_sfm = np.linalg.inv(sfm_to_gt)
            sfm_vert1 = (gt_to_sfm[:3, :3] @ np.array(self.scene_config["eval_bbx_detail"][0]) + gt_to_sfm[:3, 3])
            sfm_vert2 = (gt_to_sfm[:3, :3] @ np.array(self.scene_config["eval_bbx_detail"][1]) + gt_to_sfm[:3, 3])
            ...
            mesh_detail = extract_mesh( dim=256,
                                        chunk=16384,
                                        scene_radius=self.scene_config["radius"],
                                        scene_origin=self.scene_config["origin"],
                                        ...
                                        renderer=self.renderer)
            if self.trainer.global_rank == 0:
                mesh_detail.export(os.path.join(mesh_dir, "{:0>8d}_detail.ply".format(self.global_step) ))

            gt_path = os.path.join(self.config.DATASET.ROOT_DIR, "gt.ply")
            if os.path.exists(gt_path) and self.trainer.global_rank == 0:
                eval_metrics = eval_mesh(file_pred=os.path.join( mesh_dir, "{:0>8d}_detail.ply".format(self.global_step)),
                                        file_trgt=gt_path,
