...
class DataModule(pl.LightningDataModule):
    ...
    def __init__(self, args, config):
        ...
        self.specific_dataset_config = getattr(self.dataset_config, self.dataset_config.DATASET_NAME.upper())
    ...
    def setup(self, stage):
        ...
        if self.dataset_config.DATASET_NAME == "phototourism":
            kwargs[ "semantic_map_path" ] = self.dataset_config.PHOTOTOURISM.SEMANTIC_MAP_PATH
        ...
        if self.specific_dataset_config.USE_CACHE:
            ...
            if self.specific_dataset_config.IMG_DOWNSCALE == 1:
                self.train_dataset = self._setup_dataset(dataset, split="train", **kwargs)
            else:
    ...
    def _get_local_split(self, items: list, world_size: int, rank: int, seed: int = 6):
        ...
        if n_items % world_size == 0:
            ...
            padding = np.random.RandomState(seed).choice(items, world_size - (n_items % world_size), replace=True)
            padded_items = np.concatenate([items_permute, padding])
            assert (len(padded_items) % world_size == 0), f"len(padded_items): {len(padded_items)}; world_size: {world_size}; len(padding): {len(padding)}"
    ...
    def _setup_dataset(self, dataset, split, **kwargs):
        ...
        splits_names = next(os.walk(os.path.join(self.dataset_config.ROOT_DIR, split_path)))[1]
        
        local_splits_names = self._get_local_split(splits_names, self.world_size, self.rank)
        logger.info(f"[rank {self.rank}]: {len(local_splits_names)} npz(s) assigned. {local_splits_names}")
