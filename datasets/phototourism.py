...
class PhototourismDataset(Dataset):
    ...
    def get_colmap_depth(self,
        ...
        Err_mean = torch.mean(img_err) #pixel error
    ...
    def read_meta(self):
        ...
        #add
        #self.pair_ids = {}
        #with open(os.path.join( self.root_dir, "dense/pair.txt")) as f:
        ...
        if not self.use_cache:
            ...
            self.poses_raw_dict = {id_: self.poses_raw[i] for i, id_ in enumerate(self.img_ids)}
            #add
        ...
        if self.split in ["train", "eval"]:  # create buffer of all rays and rgb data
            if self.use_cache:
                ...
                for id_ in tqdm(img_ids_split):
                    ...
                    c2w_raw = torch.FloatTensor(self.poses_raw_dict[id_])
                    ...
                    if self.with_semantics:
                        ...
                        #read normal and uncertainty proir
                        normal_map = np.load(os.path.join(self.root_dir,
                                                            f"normal/{image_name}.npz",))["arr_0"]
                        normal_map = cv2.resize(normal_map, 
                                                (normal_map.shape[1] // self.img_downscale, 
                                                normal_map.shape[0] // self.img_downscale),
                                                interpolation=cv2.INTER_NEAREST)
                        normal_prior_ref = normal_map.reshape(-1, 3)
                        normal_prior_ref = torch.from_numpy(normal_prior_ref)
                        print("normal_prior_ref: ", normal_prior_ref)

                        normal_prior_world = normal_prior_ref @ c2w_raw[:,:3].T
                        normal_prior_world = normal_prior_world / torch.norm(normal_prior_world, dim=-1, keepdim=True)
                        normal_prior_world = normal_prior_world.numpy()
                        print("normal_prior_world: ", normal_prior_world)


                        uncertainty = np.load(os.path.join(self.root_dir,
                                                            f"uncertainty/{image_name}.npz",))["arr_0"]
                        uncertainty = cv2.resize(uncertainty, 
                                                (uncertainty.shape[1] // self.img_downscale, 
                                                uncertainty.shape[0] // self.img_downscale),
                                                interpolation=cv2.INTER_NEAREST)
                        uncertainty_prior = uncertainty.reshape(-1, 1)

                        edge_uncertainty = np.load(os.path.join(self.root_dir,
                                                            f"edge/{image_name}.npz",))["arr_0"]
                        edge_uncertainty = cv2.resize(edge_uncertainty, 
                                                (edge_uncertainty.shape[1] // self.img_downscale, 
                                                edge_uncertainty.shape[0] // self.img_downscale),
                                                interpolation=cv2.INTER_NEAREST)
                        edge_prior = edge_uncertainty.reshape(-1, 1)
                        ...
                        rays = torch.cat([rays_o,
                                            ...
                                            rays_t,#8
                                            torch.from_numpy(rays_mask), #semantic
                                            depths,#10
                                            weights,
                                            pts_sfm,#12~14
                                            id_ * torch.ones_like(rays_o[:, :1]),#15
                                            #self.pair_ids[id_] * torch.ones_like(rays_o[:, :1]),#16
                                            torch.from_numpy(normal_prior_world), #17,18,19
                                            torch.from_numpy(uncertainty_prior),#20
                                            torch.from_numpy(edge_prior),#21
                                            ], 1)  # (h*w, 0~20)
                    else:
                        rays = torch.cat([rays_o,
                                            rays_d,
    ...
    def __getitem__(self, idx):
        if self.split == "train":  # use data in the buffers
            ...
            if self.with_semantics:
                sample["semantics"] = self.all_rays[idx, 9] # 9: mask, 10~16: depth,weight,pts_sfm,ref,src
                sample["rays"] = torch.cat((self.all_rays[idx, :8], self.all_rays[idx, 10:21]), dim=-1)
            else:                                           ## 9~13:depth,weight,pts_sfm
                sample["rays"] = torch.cat((self.all_rays[idx, :8], self.all_rays[idx, 9:15]), dim=-1)
            ...
            if self.with_semantics:
                ...
                normal_map = cv2.resize( normal_map,
                                           (normal_map.shape[1] // self.img_downscale, 
                                            normal_map.shape[0] // self.img_downscale),
                                           interpolation=cv2.INTER_NEAREST)
                normal_prior = normal_map.reshape(-1, 1)
                sample["normal_prior"] = torch.from_numpy(normal_prior)   
                ...
                uncertainty = cv2.resize(uncertainty, 
                                        ...
                                        interpolation=cv2.INTER_NEAREST)
                uncertainty_prior = uncertainty.reshape(-1, 1)      
                sample["uncertainty_prior"] = torch.from_numpy(uncertainty_prior)  
                ...
                edge_uncertainty = cv2.resize(edge_uncertainty, 
                                        ...
                                        interpolation=cv2.INTER_NEAREST)
                edge_prior = edge_uncertainty.reshape(-1, 1)      
                sample["edge_prior"] = torch.from_numpy(edge_prior)        
