...
def sample_pdf(bins, weights, n_samples, det=False):
    ...
    if det:
        u = torch.linspace( 0.0 + 0.5 / n_samples, 1.0 - 0.5 / n_samples, steps=n_samples, device=device)
...
class NeuconWRenderer:
    def __init__(self,
                nerf,neuconw,
                embeddings,
                n_samples,n_importance,n_outside,up_sample_steps,
                perturb,
                origin,radius,
                s_val_base=0,
                spc_options=None,
    ...
    def get_octree(self, device):
        octree, scene_origin, scale, level = gen_octree_from_sfm(self.recontruct_path,
                                                                 self.min_track_length,
                                                                 self.voxel_size, device=device)
    ...
    def render_core_outside(self,
        ...
        a_embedded_expand = (a_embedded[:, None, :].expand(batch_size, n_samples, -1).reshape(dirs.size()[0], -1)
        ...
        weights = (alpha * torch.cumprod(torch.cat([torch.ones([batch_size, 1], device=device), 
                                                    1.0 - alpha + 1e-7], -1), -1)[:, :-1])
    ...
    def save_samples_step(self, pts, weights, save_name, dir_name="steps"):
        ...
        colors[weights < 0.1, :] = torch.tensor([0, 255, 255], dtype=colors.dtype, device=pts_world.device)
        colors[(weights > 0.1) & (weights < 0.9), :] = torch.tensor([0, 0, 255], dtype=colors.dtype, device=pts_world.device)
        colors[weights > 0.9, :] = torch.tensor([127, 0, 255], dtype=colors.dtype, device=pts_world.device)
        ...
        o3d.io.write_point_cloud(f"samples/{dir_name}/step_{self.save_step_itr}/{save_name}.ply", pcd)
    ...
    def up_sample(self, rays_o, rays_d, z_vals, sdf, n_importance, inv_s, step):
        ...
        if self.save_step_sample:
            ...
            # colored
            self.save_samples_step(pts[:, :-1].reshape(-1, 3),weights.reshape(-1,), f"{step}_colored")

            pts_new = rays_o[:, None, :] + rays_d[:, None, :] * z_samples[..., :, None]
            self.save_samples_step(pts_new.reshape(-1, 3), 
                                   torch.zeros_like(z_samples).reshape(-1,),
                                   f"{step}",dir_name="new_z")
        return z_samples
    ...
    def cat_z_vals(self, rays_o, rays_d, z_vals, new_z_vals, sdf, last=False):
        ...
        if not last:
            ...
            xx = (torch.arange(batch_size)[:, None].expand(batch_size, 
    ...
    def render_depth(self, alphas, z_vals):
        ...
        transmittance = torch.cumprod( torch.cat([torch.ones([alphas.size()[0], 1], device=alphas.device), 
                                                  1.0 - alphas + 1e-7,],-1), -1)[:, :-1]
    ...
    def get_near_far_octree(self, octree_data, rays_o, rays_d, near, far):
        ...
        voxel_near_sfm, voxel_far_sfm = get_near_far(rays_o_sfm,
                                                    rays_d,
                                                    octree,
                                                    octree_origin,
                                                    octree_scale,
        ...
        voxel_far = (voxel_far_sfm.float().reshape(-1, 1) + self.voxel_size) / self.radius
    ...
    def sparse_sampler(self, rays_o, rays_d, near, far, perturb):
        ...
        if self.nerf_far_override:
            ...
            near, far, hit_mask_sfm = self.get_near_far_octree(self.octree_data, rays_o, rays_d, near, far)
        ...
        if self.fine_octree_data is not None:
            ...
            sample_near, sample_far, hit_mask_sdf = self.get_near_far_sdf(self.fine_octree_data, rays_o, rays_d, near, far)
        ...
        if self.fine_octree_data is not None and self.boundary_samples > 0:
            ...
            bound_near_z = torch.linspace(0.0, 1.0, bound_near_num + 1, device=device)[:-1]
            bound_near_z = near + (z_vals[:, 0][:, None] - near) * bound_near_z[None, :]

            bound_far_z = torch.linspace(0.0, 1.0, bound_far_num + 1, device=device)[1:]
            bound_far_z = (z_vals[:, -1][:, None] + (far - z_vals[:, -1][:, None]) * bound_far_z[None, :])
    ...
    def render_core(self,
        ...
        a_embedded_ = a_embedded.unsqueeze(1).expand( -1, n_samples, -1)  # N_rays, N_samples, c
        ...
        if background_alpha is not None:
            if self.save_sample:
                ...
                if self.itr % 200 == 0:
                    ...
                    inside_pcd.points = o3d.utility.Vector3dVector(self.insiders.detach().cpu().numpy())
                    o3d.io.write_point_cloud(f"samples/inside_pts_sphere_{self.itr}.ply", inside_pcd)

                    outside_pcd = o3d.geometry.PointCloud()
                    outside_pcd.points = o3d.utility.Vector3dVector(self.outsiders.detach().cpu().numpy())
                    o3d.io.write_point_cloud(f"samples/outside_sphere.ply_{self.itr}.ply", outside_pcd )
            ...
            #inside, outside
            rgb = (rgb * inside_sphere[:, :, None] + background_sampled_color[:, :n_samples] * (1.0 - inside_sphere)[:, :, None])
            rgb = torch.cat([rgb, background_sampled_color[:, n_samples:]], dim=1)

            # render background
            if self.trim_sphere:
                background_alpha[:, :n_samples] = torch.zeros_like(alpha[:, :n_samples]) + background_alpha[:, :n_samples] * (1.0 - inside_sphere)
          
            transmittance_bg = torch.cumprod(torch.cat([torch.ones([batch_size, 1], device=device),
                                                        1.0 - background_alpha + 1e-7,], -1), -1)[:, :-1]
        ...
        transmittance = torch.cumprod(torch.cat([torch.ones([batch_size, 1], device=device), 
                                                 1.0 - alpha + 1e-7], -1), -1,)[:, :-1]
        ...
        sphere_transmittance = torch.cumprod(torch.cat([torch.ones([batch_size, 1], device=device),
                                                        1.0 - alpha_in_sphere + 1e-7,],-1), -1)[:, :-1]
        ...
        sdf_d = sdf.reshape(batch_size, n_samples) #(1024,24)
        prev_sdf, next_sdf = sdf_d[:, 0:-1], sdf_d[:, 1:] #(1024, 23)
        sign = prev_sdf * next_sdf
        sign = torch.where(sign <= 0, torch.ones_like(sign), torch.zeros_like(sign) )
        idx = reversed(torch.Tensor(range(1, n_samples)).cuda()) #1~23
        tmp = torch.einsum("ab, b->ab", (sign, idx))
        prev_idx = torch.argmax(tmp, 1, keepdim=True)#(batch_size, n_sample)
        next_idx = prev_idx + 1
        ...
        #generate 3D points of world
        pts_sdf0 = rays_o[:, None, :] + rays_d[:, None, :] * z_vals_sdf0[..., :, None]
        
        #world to cam
        gradients_sdf0 = self.neuconw.gradient(pts_sdf0.reshape(-1,3)).reshape(batch_size, 1, 3)
        gradients_sdf0 = gradients_sdf0 / torch.linalg.norm(gradients_sdf0, ord=2, dim=-1, keepdim=True)#(B,Sample,3)
        ...
        for i in range(batch_size):
            if i % 64 == 0:
                ...
                ref_extrinsic = torch.Tensor(poses_w2c_dict[ref_id]).float()#(3,4)
                ref_extrinsic = ref_extrinsic.to(device)
                src_extrinsic = torch.Tensor(poses_w2c_dict[src_id]).float()#(3,4)
                src_extrinsic = src_extrinsic.to(device)
                
                ref_intrinsic = torch.Tensor(intrinsics[ref_id][:3, :3]).float()
                ref_intrinsic = ref_intrinsic.to(device)
                ref_intrinsic_inv = torch.Tensor(intrinsics_inv[ref_id][:3, :3]).float()
                ref_intrinsic_inv = ref_intrinsic_inv.to(device)
                src_intrinsic = torch.Tensor(intrinsics[src_id][:3, :3]).float()
                src_intrinsic = src_intrinsic.to(device)
                src_intrinsic_inv = torch.Tensor(intrinsics_inv[src_id][:3, :3]).float()
                src_intrinsic_inv = src_intrinsic_inv.to(device)

                gradient_sdf0 = gradients_sdf0[i, :, :].permute(1, 0) #(3, 1)
                gradient_sdf0 = torch.matmul(ref_extrinsic[:3, :3], gradient_sdf0).permute(1, 0).detach()#(1,3)
                gradient_sdf0_src = gradients_sdf0[i, :, :].permute(1, 0) #(3, 1)
                gradient_sdf0_src = torch.matmul(src_extrinsic[:3, :3], gradient_sdf0_src).permute(1, 0).detach()#(1,3)

                gradients_sdf0[i, :, :] = gradient_sdf0[None ,...] #(1,1,3)
                
                pt_sdf0 = pts_sdf0[i, ...].permute(1, 0) #(3,1)
                
                project_xyz = torch.matmul(ref_extrinsic[:3, :3], pt_sdf0) 
                project_xyz = project_xyz + ref_extrinsic[:3, 3][:, None] #(3,1)
                pt_sdf0_ref = project_xyz #cam
                uv_sdf0_ref = torch.matmul(ref_intrinsic, pt_sdf0_ref)
                uv_sdf0_ref = uv_sdf0_ref[:2] / (uv_sdf0_ref[2] + 1e-8)
                
                project_xyz_src = torch.matmul(src_extrinsic[:3, :3], pt_sdf0) 
                project_xyz_src = project_xyz_src + src_extrinsic[:3, 3][:, None] #(3,1)
                pt_sdf0_src = project_xyz_src #src cam
                uv_sdf0_src = torch.matmul(src_intrinsic, pt_sdf0_src)
                uv_sdf0_src = uv_sdf0_src[:2] / (uv_sdf0_src[2] + 1e-8)
                
            
                project_xyz = torch.matmul(ref_intrinsic, project_xyz)
                depth_sdf = project_xyz[2, 0] * mid_inside_sphere[i, :] #1
                disp_sdf0 = torch.matmul(gradient_sdf0, pt_sdf0_ref).squeeze(0) #1
                
                project_xyz_src = torch.matmul(src_intrinsic, project_xyz_src)
                depth_sdf_src = project_xyz_src[2, 0] * mid_inside_sphere[i, :] #1
                disp_sdf0_src = torch.matmul(gradient_sdf0_src, pt_sdf0_src).squeeze(0) #1
                
                #input
                #H=K_s*(R_s*R_r^T - R_s(R_s^T*t_s - R_r^T * t_r)n^T / d) * K_r^-1, d = -(n^T * p)
                ref_R_inv = ref_extrinsic[:3, :3].permute(1,0) #c2w
                ref_R = ref_extrinsic[:3, :3]#w2c
                ref_T = ref_extrinsic[:3, 3]#w2c
                src_R = src_extrinsic[:3, :3] #w2c
                src_R_inv = src_extrinsic[:3, :3].permute(1, 0) #w2c
                src_T = src_extrinsic[:3, 3] #w2c
                
                #R_relative = torch.matmul(src_R, ref_R_inv) #R_s*R_r^T
                #C_relative = ref_T - src_T                 #R_s^T*t_s - R_r^T * t_r
                #tmp = torch.matmul(src_R, C_relative[:,None])
                #tmp = torch.matmul(tmp, gradient_sdf0) #3*1 * 1*3 = 3*3
                #tmp = R_relative + tmp / (disp_sdf0 + 1e-10)
                #tmp = torch.matmul(src_intrinsic, tmp)
                #home_warp = torch.matmul(tmp, ref_intrinsic_inv) #(3,3)
                            
                #H=K_s*(R_s*R_r^T - R_s(R_s^T*t_s - R_r^T * t_r)n^T / d) * K_r^-1, d = -(n^T * p)
                #R_relative1 = torch.matmul(ref_R, src_R_inv) 
                #C_relative1 = src_T - ref_T                 
                #tmp1 = torch.matmul(ref_R, C_relative1[:,None])
                #tmp1 = torch.matmul(tmp1, gradient_sdf0_src) 
                #tmp1 = R_relative1 + tmp1 / (disp_sdf0_src + 1e-10)
                #tmp1 = torch.matmul(ref_intrinsic, tmp1)
                #home_warp_1 = torch.matmul(tmp1, src_intrinsic_inv) 
                
                #ref pixel 
                #pixel_x = project_xyz[0,0] / (project_xyz[2,0] + 1e-8)
                #pixel_y = project_xyz[1,0] / (project_xyz[2,0] + 1e-8)
                #pixels = torch.stack([pixel_x, pixel_y], dim=-1).float()
                #pixels = pixels[None,:]
                
                #patch_size = 1
                #total_size = (patch_size * 2 + 1)**2
                #offsets = torch.arange(-patch_size, patch_size+1, device=rays_o.device)
                #offsets = torch.stack(torch.meshgrid(offsets, offsets)[::-1], dim=-1).view(-1, 2)
                
                #ref_patch = (pixels + offsets.float())
                #ref_patch_homo = torch.cat( [ref_patch, torch.ones_like(ref_patch[:, 0]).unsqueeze(1)], -1).permute(1, 0)
                
                #src_patch = torch.matmul(home_warp,  ref_patch_homo).permute(1, 0)
                #src_patch = src_patch[:, :2] / src_patch[:,2].unsqueeze(1)
                #src_patch_homo = torch.cat( [src_patch, torch.ones_like(src_patch[:, 0]).unsqueeze(1)], -1)
                
                #ref_patch_1 = torch.matmul(home_warp_1,  src_patch_homo.permute(1, 0)).permute(1, 0)
                #ref_patch_1 = ref_patch_1[:, :2] / ref_patch_1[:,2].unsqueeze(1)
                
                uv_sdf0_src = torch.cat( [uv_sdf0_src, torch.ones_like(uv_sdf0_src[:1, 0]).unsqueeze(1)], 0)
                tmp = torch.matmul(src_intrinsic_inv, uv_sdf0_src)
                tmp = torch.matmul(src_R_inv, tmp) - torch.matmul(src_R_inv, src_T.unsqueeze(1))
                tmp = torch.matmul(ref_R, tmp) + ref_T.unsqueeze(1)
                ref_d_1 = tmp[2]
                ref_d = pt_sdf0_ref[2]
                uv_sdf0_ref_1 = torch.matmul(ref_intrinsic, tmp)
                uv_sdf0_ref_1 = uv_sdf0_ref_1[:2] / (uv_sdf0_ref_1[2].unsqueeze(1) + 1e-8)
                
                pixel_error_uv = torch.abs(uv_sdf0_ref_1 - uv_sdf0_ref)
                pixel_error_uv = torch.pow(pixel_error_uv, 2)
                print("pixel_error_uv: ", pixel_error_uv)
                pixel_error_uv = pixel_error_uv.sum()
                rint("pixel_error_uv: ", pixel_error_uv)
                pixel_dist = torch.sqrt(pixel_error_uv)
                rint("pixel_error_uv: ", pixel_error_uv)
                pixel_dist_array.append(pixel_dist)

                #depth_error = torch.abs(ref_d_1 - ref_d) / torch.abs(ref_d)
                #depth_error_array.append(depth_error)
            
            else:
                #depth_error_array.append(999.0)
                pixel_dist_array.append(999.0)

        #depth_error_array = torch.Tensor(depth_error_array).to(device)
        pixel_dist_array = torch.Tensor(pixel_dist_array).to(device)
        ...
        return {
            ...
            "gradients": gradients,
            "normals": normals,
            #"depth_reproj_error":depth_error_array.unsqueeze(1),
            "pixel_dist": pixel_dist_array.unsqueeze(1),
        }
    ...
    def render(self,
        ...
        if rays.size()[1] >= 10:
            #depth_gt, depth_weight, pts_sfm, ref_ids, src_ids,normals_prior_ref, uncertaintys_prior,edge_prior = rays[:, 8], rays[:, 9],rays[:, 10:13], rays[:, 13], rays[:, 14],rays[:, 15:18],rays[:, 18],rays[:, 19]
            depth_gt, depth_weight, pts_sfm, ref_ids,normals_prior_ref, uncertaintys_prior,edge_prior = rays[:, 8], rays[:, 9],rays[:, 10:13], rays[:, 13],rays[:, 14:17],rays[:, 17],rays[:, 18]
        else:
            depth_gt = depth_weight = torch.zeros_like(near).squeeze()
            pts_sfm = torch.zeros_like(rays_o).squeeze()
            ref_ids = torch.ones_like(near).squeeze()
            #src_ids = 3 * torch.ones_like(near).squeeze() #ref:0, src:2
            normals_prior_ref = torch.zeros_like(rays_o).squeeze() + torch.randn(1, 3).to(rays_o.device)
            uncertaintys_prior = torch.zeros_like(near).squeeze() + torch.randn(1).to(rays_o.device)
            edge_prior = torch.zeros_like(near).squeeze() + torch.randn(1).to(rays_o.device)
        
        uncertaintys_prior = uncertaintys_prior.unsqueeze(1)
        edge_prior = edge_prior.unsqueeze(1)
        ...
        pts_sfm = (pts_sfm / self.radius).float()
        ...
        with torch.no_grad():
            n_samples, z_vals, z_vals_outside, sample_dist = self.sparse_sampler(rays_o, rays_d, near, far, perturb)
        ...
        if self.render_bg and self.n_outside > 0:
            ...
            ret_outside = self.render_core_outside(rays_o,
                                                   rays_d,
                                                   z_vals_feed,
                                                   sample_dist,
                                                   self.nerf,
        ...
        if self.mesh_mask_list is not None:
            ...
            mask_error = F.binary_cross_entropy(weights_sum.clip(1e-3, 1.0 - 1e-3), mask, reduction="none")
        else:
        ...
        if self.floor_normal:
            floor_normal_error, floor_y_error = self.floor_loss(label, normals, rays_o, rays_d, rendered_depth)
        else:
            floor_normal_error, floor_y_error = torch.zeros_like(normals), torch.zeros_like(normals)

        # depth error
        if self.depth_loss and torch.sum(depth_weight > 0) > 0:     
            mask_depth = depth_weight > 0
            depth_weight_valid = depth_weight[mask_depth]
            pts_sfm_valid = pts_sfm[mask_depth]

            sdf_sfm = self.sdf(pts_sfm_valid)
            gradient_sfm = self.gradient(pts_sfm_valid)
            gradient_sfm = gradient_sfm / torch.linalg.norm(gradient_sfm, ord=2, dim=-1, keepdim=True)#(B,3)
            
            #pts_sfm_valid_1 = pts_sfm_valid - sdf_sfm * gradient_sfm                      
            #sdf_sfm_1 = self.sdf_offset(pts_sfm_valid_1)      
            pts_sfm_valid_1 = sdf_sfm * gradient_sfm 
            sdf_sfm_1 = self.sdf_offset(pts_sfm_valid_1)      
            
            sdf_sfm = sdf_sfm - sdf_sfm_1    

            #sfm_sdf_loss = F.l1_loss(sdf_sfm_1, torch.zeros_like(sdf_sfm_1), reduction='none') * depth_weight_valid
            sfm_sdf_loss = F.l1_loss(sdf_sfm, torch.zeros_like(sdf_sfm), reduction='none') * depth_weight_valid
            sfm_depth_loss = (((rendered_depth - depth_gt) ** 2) * depth_weight)[depth_weight > 0]
        else:
            sfm_depth_loss = torch.zeros_like(rendered_depth)
            sfm_sdf_loss = torch.zeros_like(rendered_depth)
        ...
        return {
            ...
            "sfm_sdf_loss":sfm_sdf_loss,
            #"depth_reproj_error":ret_fine["depth_reproj_error"],
            "pixel_dist_error":ret_fine["pixel_dist"],
            "normals":normals,
            "normals_prior":normals_prior_ref,
            "uncertaintys_prior":uncertaintys_prior,
    ...
    def gradient(self, pts):
        gradients = self.neuconw.gradient(pts)
        return gradients

    def sdf_offset(self, pts):
        sdf_offset = self.neuconw.id_sdf(pts)
        return sdf_offset
