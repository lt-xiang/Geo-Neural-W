...
class SDFNetwork(nn.Module):
    def __init__(
        ...
        self.num_layers = len(dims)#10 layers
        self.skip_in = skip_in
        self.scale = scale
        self.multires = multires

        for l in range(0, self.num_layers - 1):#0~8,
...
class IDNetwork(nn.Module):
    def __init__(
        ...
        dims = [d_in] + [d_hidden for _ in range(n_layers)] + [d_out]# dim=6
        ...
        self.num_layers = len(dims)#6 layers
        self.skip_in = skip_in
        self.scale = scale
        self.multires = multires

        for l in range(0, self.num_layers - 1):#0~4,
            if l + 1 in self.skip_in:
                out_dim = dims[l + 1] - dims[0]
            else:
                out_dim = dims[l + 1]

            lin = nn.Linear(dims[l], out_dim)#0,1,2,3,4
...
class NeuconW(nn.Module):
    ...
    def id_sdf(self, input_xyz):
        
        return self.id_sdf_net.sdf(input_xyz)  # (B, w+1)
