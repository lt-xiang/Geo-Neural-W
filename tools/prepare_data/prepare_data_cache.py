...
def get_opts():
    ...
    parser.add_argument("--root_dir", type=str, default="../../dataset/heritage-recon/brandenburg_gate", help="root directory of dataset")
    parser.add_argument(
        "--dataset_name",
        type=str,
...
if __name__ == "__main__":
    ...
    #extra information: intrinsics, intrinsics_inv, poses, pair_ids
    with open(os.path.join(args.root_dir, f"{args.cache_dir}/intrinsics_all.pkl"), 'wb') as f:
        pickle.dump(dataset.Ks, f, pickle.HIGHEST_PROTOCOL)
    with open(os.path.join(args.root_dir, f"{args.cache_dir}/intrinsics_inv_all.pkl"), 'wb') as f:
        pickle.dump(dataset.Ks_inv, f, pickle.HIGHEST_PROTOCOL)
    with open(os.path.join(args.root_dir, f"{args.cache_dir}/poses_w2c_dict_all.pkl"), 'wb') as f:
         pickle.dump(dataset.poses_w2c_dict, f, pickle.HIGHEST_PROTOCOL)
    ...
    if args.split_to_chunks > 0:
        ...
        print(f"Total length: {all_lengths}, Padding size: {padding_size}, Trunk length: {chunk_length}")
        print(f"Splitting rgb...")
        split_to_chunks(rgbs, all_lengths, chunk_length, split_path, args, padding_index, "rgbs")
        rays = dataset.all_rays
        print(f"Splitting ray...")
        split_to_chunks(rays, all_lengths, chunk_length, split_path, args, padding_index, "rays")
    else:
        start_time = time.time()
        if args.cache_type == "h5":
            
            rays_file = os.path.join(args.root_dir, f"{args.cache_dir}/rays{args.img_downscale}.h5")
            with h5py.File(rays_file, "w") as f:
                f.create_dataset("rays", data=torch.cat(dataset.all_rays).numpy(), chunks=True)
                
            rgbs_file = os.path.join(args.root_dir, f"{args.cache_dir}/rgbs{args.img_downscale}.h5")
            with h5py.File(rgbs_file, "w") as f:
                f.create_dataset("rgbs", data=torch.cat(dataset.all_rgbs).numpy(), chunks=True)
        else:
            np.savez_compressed(os.path.join(args.root_dir, f"{args.cache_dir}/rays{args.img_downscale}.npz"),torch.cat(dataset.all_rays).numpy())
            np.savez_compressed( os.path.join(args.root_dir, f"{args.cache_dir}/rgbs{args.img_downscale}.npz"),torch.cat(dataset.all_rgbs).numpy())
