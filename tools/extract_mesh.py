...
if __name__ == "__main__":
    ...
    mesh = extract_mesh(args.mesh_size, args.chunk, 
                        scene_config['radius'], scene_config['origin'],
                        origin = args.mesh_origin, 
                        radius = args.mesh_radius, 
                        with_color = args.vertex_color,
                        embedding_a = neuconw_sys.embedding_a((torch.ones(1, device=neuconw_sys.device) * 1123).long()),
                        chunk_rgb = args.chunk_rgb, 
                        sparse_data = sparse_data, 
                        renderer = neuconw_sys.renderer)
